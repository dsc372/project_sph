  ...
  <div class="trade-container">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        ...
        <span>联系电话:{{ finalAddress.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      try {
        await this.$store.dispatch("order/submitOrder", { tradeNo, data });
        this.$router.push(`/pay?orderId=${this.$store.state.order.orderId}`);
      } catch (error) {
        alert(error);
      }
    },
    async getUserAddressAndOrderInfo() {
      try {
        await this.$store.dispatch("trade/getUserAddress");
        //由于数据中没有一个address的isDefalut为1，此处手动设置
  ...
  mounted() {
    this.getUserAddressAndOrderInfo();
  },
};
</script>
